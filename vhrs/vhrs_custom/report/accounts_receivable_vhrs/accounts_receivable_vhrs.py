...
class ReceivablePayableReport(object):
    def __init__(self, filters=None):
        ...
        self.filters.report_date = getdate(
            self.filters.report_date or nowdate())
        self.age_as_on = getdate(nowdate()) \
            if self.filters.report_date > getdate(nowdate()) \
            else self.filters.report_date

    def run(self, args):
        party_naming_by = frappe.db.get_value(
    ...
    def get_columns(self, party_naming_by, args):
        ...
        columns = [_("Posting Date") + ":Date:80", _(args.get("party_type")
                                                     ) + ":Link/" + args.get("party_type") + ":200"]
        ...
        for label in ("0-{range1}".format(range1=self.filters["range1"]),
                      "{range1}-{range2}".format(range1=cint(
                          self.filters["range1"]) + 1, range2=self.filters["range2"]),
                      "{range2}-{range3}".format(range2=cint(
                          self.filters["range2"]) + 1, range3=self.filters["range3"]),
    ...
    def get_data(self, party_naming_by, args):
        ...
        future_vouchers = self.get_entries_after(
            self.filters.report_date, args.get("party_type"))
            ...
            if self.is_receivable_or_payable(gle, dr_or_cr, future_vouchers):
                ...
                if abs(outstanding_amount) > 0.1 / 10**currency_precision:
                    ...
                    if args.get("party_type") == "Supplier":
                        row += [
                            voucher_details.get(
                                gle.voucher_no, {}).get("bill_no", ""),
                            voucher_details.get(
                                gle.voucher_no, {}).get("bill_date", "")
                        ]
                    ...
                    row += [invoiced_amount, paid_amt,
                            credit_note_amount, outstanding_amount]
    ...
    def prepare_conditions(self, party_type):
        ...
        if party_type_field == "customer":
            ...
            if self.filters.get("credit_days_based_on"):
                conditions.append(
                    "party in (select name from tabCustomer where credit_days_based_on=%s)")
