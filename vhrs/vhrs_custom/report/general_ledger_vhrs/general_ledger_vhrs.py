...
def validate_filters(filters, account_details):
    ...
    if filters.get("account") and filters.get("group_by_account") \
        ...
        frappe.throw(
            _("Can not filter based on Account, if grouped by Account"))

    if filters.get("voucher_no") and filters.get("group_by_voucher"):
        frappe.throw(
            _("Can not filter based on Voucher No, if grouped by Voucher"))
...
def validate_party(filters):
    ...
    if party:
        if not party_type:
            frappe.throw(
                _("To filter based on Party, select Party Type first"))
        ...
        if filters.get("account"):
            ...
            if gle_currency:
                ...
                account_currency = None if filters.party_type == "Employee" else \
                    frappe.db.get_value(filters.party_type,
                                        filters.party, "default_currency")
...
def get_columns(filters):
    ...
    columns += [
        ...
        _("Against Voucher Type") + "::120", _("Against Voucher") +
        ":Dynamic Link/" + _("Against Voucher Type") + ":160",
        _("Remarks") + "::400"
    ]
...
def get_data_with_opening_closing(filters, account_details, gl_entries):
    ...
    opening, total_debit, total_credit, opening_in_account_currency, total_debit_in_account_currency, \
        total_credit_in_account_currency, gle_map = get_accountwise_gle(
            filters, gl_entries, gle_map)

    # Opening for filtered account
    if filters.get("account") or filters.get("party"):
        data += [get_balance_row(_("Opening"), opening,
                                 opening_in_account_currency), {}]
    ...
    if filters.get("group_by_account"):
        for acc, acc_dict in gle_map.items():
            if acc_dict.entries:
                ...
                # Totals and closing for individual ledger, if grouped by
                # account
...
def get_accountwise_gle(filters, gl_entries, gle_map):
    ...
    for gle in gl_entries:
        ...
        amount_in_account_currency = flt(
            gle.debit_in_account_currency, 3) - flt(gle.credit_in_account_currency, 3)
        ...
        if (filters.get("account") or filters.get("party") or filters.get("group_by_account")) \
            ...
            if filters.get("show_in_account_currency"):
                gle_map[gle.account].total_debit_in_account_currency += flt(
                    gle.debit_in_account_currency, 3)
                gle_map[gle.account].total_credit_in_account_currency += flt(
                    gle.credit_in_account_currency, 3)

                total_debit_in_account_currency += flt(
                    gle.debit_in_account_currency, 3)
                total_credit_in_account_currency += flt(
                    gle.credit_in_account_currency, 3)
